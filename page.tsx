...
import { Shield, Eye, Radar, ScanLine, Lock, Bell, Globe2, ArrowRight, CheckCircle2, Sparkles, Building2, Megaphone, Siren, ShieldAlert } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/60 border-b border-white/10">
      <div className="mx-auto max-w-7xl px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-400 grid place-items-center shadow-lg shadow-violet-500/30">
            <Shield className="h-5 w-5 text-neutral-950" />
          </div>
          <span className="font-semibold tracking-tight text-xl">Zarn</span>
        </div>
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 md:py-28 grid md:grid-cols-2 gap-10 items-center">
        <div>
          <motion.h1 initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-4xl md:text-6xl font-semibold tracking-tight">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="relative">
...
function DemoRow({ icon, label, value, severity }: { icon: React.ReactNode; label: string; value: string; severity: "Low"|"Medium"|"High"|"Critical" }) {
  const badge = {
    ...
    High: "bg-violet-500/20 text-orange-200 border-orange-500/30",
  ...
  return (
    <footer className="border-t border-white/10">
      <div className="mx-auto max-w-7xl px-6 py-12 grid md:grid-cols-3 gap-10 text-sm">
        <div>
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-400 grid place-items-center">
              <Shield className="h-4 w-4 text-neutral-950" />
            </div>
            <span className="font-semibold">Zarn</span>
          </div>
